    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #이더리움 자동매매 코드 09:00~08:59
            ...
            if target_price < current_price <target_price*1.01 :
                krw = get_balance("KRW")
                if krw > 150000 and eb1==0:
                    eb1=1 #구매 확인코드
                    upbit.buy_market_order("KRW-ETH", 500000) #30만원 매수 코드
            elif current_price < target_price*0.94 and eb1==1: #손절라인 만들기 코드
                eth = get_balance("ETH")
                if eth > 0.002:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #리플 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price1 < current_price1<target_price1*1.01:
                krw = get_balance("KRW")
                if krw > 150000 and eb2==0:
                    eb2=1
                    upbit.buy_market_order("KRW-XRP", 500000)
            elif current_price < target_price*0.94 and eb2==1 :
                xrp = get_balance("XRP")
                if xrp > 4:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #비트토렌트 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price2 < current_price2<target_price2*1.01:
                krw = get_balance("KRW")
                if krw > 150000 and eb3==0:
                    eb3=1
                    upbit.buy_market_order("KRW-BTT", 500000)
            elif current_price < target_price*0.94 and eb3==1 :
                btt = get_balance("BTT")
                if btt > 1000:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #비체인 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price3 < current_price3<target_price3*1.01:
                krw = get_balance("KRW")
                if krw > 150000 and eb4==0:
                    eb4=1
                    upbit.buy_market_order("KRW-VET", 500000)
            elif current_price < target_price*0.94 and eb4==1 :
                vet = get_balance("VET")
                if vet > 30 :
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #도지 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price4 < current_price4<target_price4*1.01:
                krw = get_balance("KRW")
                if krw > 150000 and eb5==0:
                    eb5=1
                    upbit.buy_market_order("KRW-DOGE", 500000)
            elif current_price < target_price*0.94 and eb5==1 :
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #이더리움 클래식 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price5 < current_price5<target_price5*1.01:
                if krw > 150000 and eb6==0:
                    eb6=1
                    upbit.buy_market_order("KRW-ETC", 500000)
            elif current_price < target_price*0.94 and eb6==1 :
                etc = get_balance("ETC")
                if etc > 0.2:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #비트코인골드 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price6 < current_price6 < target_price6*1.01:
                krw = get_balance("KRW")
                if krw > 5000 and eb7==0:
                    eb7=1
                    upbit.buy_market_order("KRW-BTG", 500000)
            elif current_price < target_price*0.94 and eb7==1 :
                btg = get_balance("BTG")
                if btg > 0.05:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #칠리즈 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price7 < current_price7 < target_price7*1.01: 
                krw = get_balance("KRW")
                if krw > 5000 and eb8==0:
                    eb8=1
                    upbit.buy_market_order("KRW-CHZ", 500000)
            elif current_price < target_price*0.94 and eb8==1 :
                chz = get_balance("CHZ")
                if chz > 10:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=60): #메디블록 자동매매 코드, 자산의 13프로, K=0.5
            ...
            if target_price8 < current_price8 < target_price8*1.01: 
                krw = get_balance("KRW")
                if krw > 150000 and eb9==0:
                    eb9=1
                    upbit.buy_market_order("KRW-MED", 500000)
            elif current_price < target_price*0.94 and eb9==1 :
                chz = get_balance("MED")
                if med > 30:
